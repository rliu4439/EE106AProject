...
#### MPC with LQR
A Linear Quadratic Regulator (LQR) was originally used to solve the optimal control problem of driving the car to a desired waypoint. This was done repeatedly for every control step giving us a Model Predictive Control (MPC) scheme, but it had to be heavily modified in order to add the additional constraint required to implement the safety feautures we planned. Initally, the optimal controller was calculated via the Ricatti equation, but this was no longer possible with the aforementioned constraint, so we adopted a different approach:
